<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Edit User</h1>
      </v-col>
    </v-row>
        ...
        <v-card>
          <v-card-text>
            <AutoSaveField
              ...
              automation-id="user-edit-name-input"
            />

            <AutoSaveField
              :model-value="user.description || ''"
              label="Description"
              ...
              textarea
              :rows="3"
              automation-id="user-edit-description-input"
            />
            ...
            <AutoSaveSelect
              ...
              class="mt-4"
              automation-id="user-edit-status-select"
            />
            ...
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(user.created.at_time)"
                  label="Created"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="user.created.by_user"
                  label="Created By"
                  readonly
                  variant="outlined"
                  density="compact"
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(user.saved.at_time)"
                  label="Last Saved"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="user.saved.by_user"
                  label="Last Saved By"
                  readonly
                  variant="outlined"
                  density="compact"
            ...
            <v-card-actions class="px-0 mt-4">
              <v-btn 
                ...
                variant="text"
                data-automation-id="user-edit-back-button"
              >
                Back to List
              </v-btn>
            </v-card-actions>
...
/**
 * User Edit Page - Showcase of spa_utils AutoSave components
 * 
...
const userId = computed(() => routeLocation.params.id as string)
...
const { mutateAsync: updateUser } = useMutation({
  mutationFn: (data: UserUpdate) => api.updateUser(userId.value, data),
  onSuccess: () => {
...
async function updateField(field: keyof UserUpdate, value: string) {
  try {
